...
cg_script = [path.abspath("cg\\background\\bg_000"), path.abspath("cg\\background\\bg_001"), path.abspath("cg\\background\\bg_002"), path.abspath("cg\\background\\bg_003"), path.abspath("cg\\background\\bg_004"), path.abspath("cg\\background\\bg_005"), path.abspath("cg\\background\\bg_006"), path.abspath("cg\\background\\bg_007"), path.abspath("cg\\background\\bg_008"), path.abspath("cg\\background\\bg_009")]
...
class VisualGame(object): 
	def __init__(self):
		...
		self.savefolder1_button = Button(self.saveFrame, text = "        Save 1", height = 100, width = 180, 
										 ...
										 padx = 10, pady = 10, relief = GROOVE, command = self.save(path.abspath("saves\\save1.txt")))
		...
		self.savefolder2_button = Button(self.saveFrame, text = "        Save 2", height = 100, width = 180, 
										 ...
										 padx = 10, pady = 10, relief = GROOVE, command = self.save(path.abspath("saves\\save2.txt")))
		...
		self.loadfolder1_button = Button(self.loadFrame, text = "        File 1", height = 100, width = 180, 
										 ...
										 padx = 10, pady = 10, relief = GROOVE, command = self.load(path.abspath("saves\\save1.txt")))
		...
		self.loadfolder2_button = Button(self.loadFrame, text = "        File 2", height = 100, width = 180, 
										 ...
										 padx = 10, pady = 10, relief = GROOVE, command = self.load(path.abspath("saves\\save2.txt")))
	...
	def load(self, lines):
		self.cg_var = cg_script[int(lines[0])]	#It has a problem with this for some reason
		self.text_var = text_script[int(lines[1])]	#
		self.char_var = char_script[int(lines[2])]
	
	def next_frame(self, decision, counter_num):
		if decision == 1:
			self.cg_counter = counter_num
			self.cg_var = cg_script[self.cg_counter]
		elif decision == 2:
			self.text_counter = counter_num
